...
const MovieForm = () => {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API_URL}/Vendors/fetch_City`, {
        params: { username },
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API_URL}/Vendors/getMovies`, {
        params: { username },
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  const fetchLocations = (selectedCity) => {
    axios
      .get(`${process.env.REACT_APP_API_URL}/Vendors/getAreas`, {
  ...
  const fetchTheatres = (selectedCity, selectedArea) => {
    axios
      .get(`${process.env.REACT_APP_API_URL}/Vendors/getTheatres`, {
  ...
  const fetchScreens = (selectedTheatre) => {
    ...
    axios
      .get(`${process.env.REACT_APP_API_URL}/Vendors/getScreens`, {
        params: { theatre: selectedTheatre },
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((response) => {
        const numberOfScreens = Number(response.data);
        const screensArray = Array.from(
          ...
          <div className="mov-form-group">
            ...
            >
              ...
              {availableTheatres.map((theatre, index) => (
                <option key={theatre.theatre_id} value={theatre.theater_id}>
                  {theatre.name}
                </option>
              ))}
            </select>
